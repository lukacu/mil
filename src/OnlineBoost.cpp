...
Matrixf				ClfStrong::applyToImage(ClfStrong *clf, Matrixu& img, bool logR)
{
    ...
    SampleSet x;
    x.sampleImage(&img, 0, 0, width, height, 100000); // sample every point
...
void				ClfMilBoost::update(SampleSet& posx, SampleSet& negx)
{
    ...
    for (int j = 0; j < pospred[i].size(); j++) {
    	printf(", %f ", pospred[j][i]);
    } printf("\n "); }

    for (int i = 0; i < negpred.size(); i++) {
    for (int j = 0; j < negpred[i].size(); j++) {
    	printf(", %f ", negpred[j][i]);
    } printf("\n "); }
    */
