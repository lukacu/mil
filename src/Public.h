...
public:
    ...
    const bool operator< (const SortableElement b)
    {
        return (_val > b._val);
    };
};
...
public:
    ...
    const bool operator< (const SortableElementRev b)
    {
        return (_val < b._val);
    };
};

template<class T> bool compare_asc(const SortableElement<T> a, const SortableElement<T> b)
{
    return (a._val > b._val);
};

template<class T> bool compare_dsc(const SortableElementRev<T> a, const SortableElementRev<T> b)
{
    return (a._val < b._val);
};
...
//            qsort(local_sampled_positions, samples, sizeof(PatchCostPair), compare_patch_cost_pair);
    std::sort(v2.begin(), v2.end(), compare_asc<T>);
    for(uint i = 0; i < n; i++)
    {
        order[i] = v2[i]._ind;
        v[i] = v2[i]._val;
...
template<class T> void				sort_order_des(vector<T>& v, vectori& order)
{
    ...
    std::sort(v2.begin(), v2.end(), compare_dsc<T>);
    for(uint i = 0; i < n; i++)
    {
        order[i] = v2[i]._ind;
        v[i] = v2[i]._val;
